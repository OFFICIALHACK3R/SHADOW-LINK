...
import { Shield, Key, Lock, Send, LogOut, UserPlus, Bot, User, Terminal, Cpu, Globe, Wifi, Activity } from 'lucide-react';
...
export default function App() {
    ...
    setTimeout(() => {
      NetworkService.resolveCode(addCodeInput.toUpperCase(), async (code, pubKeyStr, username) => {
        ...
        try {
          ...
          setContacts(prev => prev.find(c => c.id === newContact.id) ? prev : [...prev, newContact]);
  ...
  const sendMessage = async () => {
    if (!messageInput.trim() || !activeContactId || !identity) return;
              ...
              {currentMessages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id} className="hover:bg-terminal-green/5 p-1 -mx-2 px-2 transition-colors">
                    ...
                    <span className={`whitespace-pre-wrap ${isMe ? 'text-gray-300' : 'text-terminal-greenDim'}`}>
                      {msg.content}
                    </span>
